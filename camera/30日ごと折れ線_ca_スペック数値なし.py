...
# 【修正点】変数名をスペック(inter)から検索トレンド(search)を強調した名称に変更
inter_vars = ['search_res_season', 'search_vol_season', 'search_pri_season', 'search_zoo_season']
...
for asin in asins:
    ...
    # 【修正箇所】スペック数値(ln_f_val)を掛け合わせから削除
    # 「各機能の検索トレンド」×「カテゴリトレンド」のみで需要の変化を捉える
    for key in ['res', 'vol', 'pri', 'zoo']:
        ln_z_val = np.log(df[trend_map[key]] + eps)
        new_cols[f'search_{key}_season'] = ln_z_val * ln_cat
...
# --- 重み（標準化回帰係数）の出力 ---
print("\n" + "="*50)
print("提案モデル：標準化回帰係数（スペック数値を排した検索トレンドの影響度）")
print("="*50)
...
if not err_df.empty:
    ...
    ax1.set_title('機能検索トレンド（スペック除外後）による予測誤差推移', fontsize=14, pad=15)
