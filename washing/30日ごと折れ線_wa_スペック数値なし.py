...
market_main_vars = ['ln_f_sum'] # カテゴリ全体のトレンド（共通季節性）
# 【変更点】名称を「スペック考慮」から「検索トレンド単体」の意図を込めて search_xx に変更
inter_vars = ['search_cap', 'search_aut', 'search_pri', 'search_siz']
...
# 3. データ統合と前処理
# ==========================================
# ※スペック数値は計算に使わなくなりますが、ASINの紐付けチェック用に読み込みは維持します
...
for asin in asins:
    ...
    # 相対売上への変換 (Hollenbeck and He, 2020)
    k_c, A_c = 0.684, 11.74
    ...
    # 【変更点】スペック数値を掛け合わせず、検索トレンド(×カテゴリ季節性)の対数のみを抽出
    # これにより、ランキングに対する「関心の高まり」の影響のみを純粋に測定する
    new_cols['search_cap'] = np.log(df['adj_f_cap'] + eps)
    new_cols['search_aut'] = np.log(df['adj_f_aut'] + eps)
    new_cols['search_pri'] = np.log(df['adj_f_pri'] + eps)
    new_cols['search_siz'] = np.log(df['adj_f_siz'] + eps)
...
# 標準化回帰係数の出力
print("\n" + "="*50)
print("提案モデル：標準化回帰係数 (スペック数値を排した純粋な検索影響度)")
print("="*50)
...
if not err_df.empty:
    ...
    ax1.set_title('相対売上と予測誤差推移 (機能検索トレンドのみ考慮)', fontsize=title_size, pad=20)
