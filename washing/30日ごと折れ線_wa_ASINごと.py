...
asin_mae_df = pd.DataFrame(asin_mae_results)
asin_mae_sorted = asin_mae_df.sort_values('MAE_M2')

def print_mae_table(title, df):
    print(f"\n【{title}】")
    print("-" * 85)
    print(f"{'ASIN':<15} | {'MAE_M0':>8} | {'MAE_M1':>8} | {'MAE_M2':>8} | {'M2改善率':>10}")
    print("-" * 85)
    for _, row in df.iterrows():
        print(f"{row['ASIN']:<15} | {row['MAE_M0']:8.4f} | {row['MAE_M1']:8.4f} | {row['MAE_M2']:8.4f} | {row['M2改善率(%)']:9.2f}%")

print_mae_table("精度が良い ASIN 上位10 (MAE_M2基準)", asin_mae_sorted.head(10))
print_mae_table("精度が低い ASIN 下位10 (MAE_M2基準)", asin_mae_sorted.tail(10))

print("\n" + "="*85)
print(f"{'全ASIN平均':<15} | {asin_mae_df['MAE_M0'].mean():8.4f} | {asin_mae_df['MAE_M1'].mean():8.4f} | {asin_mae_df['MAE_M2'].mean():8.4f} | {asin_mae_df['M2改善率(%)'].mean():9.2f}%")
print("="*85)
...
if not err_df.empty:
    ...
    ax1.set_title('洗濯機：相対売上と精度推移（ASIN全体平均）', pad=20)
