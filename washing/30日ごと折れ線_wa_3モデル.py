...
# ラグ変数の仕分け
youngest_lag = lag_vars[0]   # 一番若いラグ (ln_y_lag7)
other_lags = lag_vars[1:]     # それ以外のラグ (lag8-30)

# 1. 一番若いラグの表示
if youngest_lag in model_m1.params:
...
# 2. それ以外のラグの平均重みを計算して表示
if other_lags:
    m0_other_avg = model_m0.params[other_lags].mean()
    m1_other_avg = model_m1.params[other_lags].mean()
    m2_other_avg = model_m2.params[other_lags].mean()
    print(f"{'ラグ平均 (lag8-30)':<25} | {m0_other_avg:10.4f} | {m1_other_avg:10.4f} | {m2_other_avg:10.4f}")

# 3. 市場全体変数 (ln_f_sum) の表示
for var in market_main_vars:
...
if not err_df.empty:
    ...
    ax1.set_title('洗濯機：相対売上と精度推移', fontsize=title_size, pad=20)
